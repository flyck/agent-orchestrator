    ...
    try {
      ...
      while (!this.aborted) {
        ...
        for (const line of lines) {
          ...
          // Server-level events (server.connected/heartbeat) are ignored
          // for now; could be exposed via a secondary subscribe('*').
        }
      }
    } catch (err) {
      if (!this.aborted) this.onError?.(err);
    } finally {
      // Bus is dead — close all queues so iterators terminate.
      for (const q of this.queues.values()) q.close();
      this.queues.clear();
    }
  }
